...
export function CareCoordinatorSection() {
  ...
  const getSectionProgress = (section: CareCoordinatorSection) => {
    let completed = 0
    let total = 0
    ...
    for (const item of section.items) {
      if (item.type === 'checkbox') {
        total++
        const itemData = careCoordinatorItems?.[item.id]
        ...
        for (const subItem of item.subItems) {
          total++
          if (careCoordinatorItems?.[item.id]?.subItems?.[subItem.id]) {
            completed++
          }
        }
        ...
        for (const trackingItem of item.assessmentTrackingItems) {
          total++
          const tracking = careCoordinatorItems?.[item.id]?.assessmentTracking?.[trackingItem.id]
